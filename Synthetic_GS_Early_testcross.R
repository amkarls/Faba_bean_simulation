...
if (exists("rep_id", inherits = FALSE) && exists("workdir", inherits = FALSE)) {
  ...
  if (length(args) < 1) {
    stop("Usage: GS_synthetic_BGLR_progenytest_V2_DD1.R <rep_id> [workdir]")
  }
...
model.mse <- function(rep_id) {
  ...
  ## -------------------------------------------------------------------
  predictEBV_BGLR <- function(pop, p_tr, bA, bD = NULL, mu = 0) {
    G <- as.matrix(pullSnpGeno(pop))
  ...
  # GCA as training data
  # Step 1. Simulate testcrosses for each training pop ind.
  training <- setPhenoProgTest(
    training,
    tester_pop,
    nMatePerInd = 50,
    varE = varE,
    reps = reporiginaltraining
  )
  
  
  ## -------------------------------------------------------------------
  ## INITIAL TRAINING CONTAINERS
  ## -------------------------------------------------------------------
  ...
  Gtr <- as.matrix(pullSnpGeno(training_all))
  storage.mode(Gtr) <- "numeric"
  
  p_tr <- colMeans(Gtr) / 2
  q_tr <- 1 - p_tr
  
  for (k in seq_along(training_by_year)) {
    pop_k <- training_by_year[[k]]
    
    Gk <- as.matrix(pullSnpGeno(pop_k))
    storage.mode(Gk) <- "numeric"
    stopifnot(ncol(Gk) == length(p_tr))
    
    A_blk <- sweep(Gk, 2, 2 * p_tr, "-")
    het   <- (Gk == 1) * 1
    D_blk <- sweep(het, 2, 2 * p_tr * q_tr, "-")
    
    genoA <- rbind(genoA, A_blk)
    genoD <- rbind(genoD, D_blk)
    
    y <- c(y, as.numeric(pop_k@pheno))
  }
  
  ETA <- list(
    list(X = genoA, model = "BRR")
    # ,list(X = genoD, model = "BRR")  # add later when you want dominance
  )
  ...
  for (year in 1:nYears) {
    ...
    if (year == 1) {
      ...
      # This activity is done during burnin
      # Step 1. Simulate open pollination to produce one synthetic line per training pop ind.
      ...
      # Apply genomic selection on F3, selecting best lines
      S3 <- lapply(S3, function(F3) {
        predictEBV_BGLR(F3, p_tr, bA = bA, bD = bD, mu = mu)
      })
      ...
      S2 <- lapply(assigned_groups, function(group) {
        predictEBV_BGLR(group, p_tr, bA = bA, bD = bD, mu = mu)
      })
      ...
      # Use synthetic variety performance data from S4
      ...
      # Simulate testcrosses with each individual line
      for (group in 1:length(S5_training)) {
        S5_training[[group]] <- setPhenoProgTest(
          S5_training[[group]],
          tester_pop,
          nMatePerInd = 50,
          varE = varE,
      ...
      # Simulate testcrosses with each individual line in added training
      add_training <- setPhenoProgTest(
        add_training,
        tester_pop,
        nMatePerInd = 50,
        varE = varE,
      ...
      for (k in seq_along(training_by_year)) {
        ...
        # centered dominance (het indicator, centered on 2pq)
        het   <- (Gk == 1) * 1
        D_blk <- sweep(het, 2, 2 * p_tr * q_tr, "-")
      ...
      S5twoyearsago <- S5  # Only for initial cycle as no S5lastyear
      S5lastyear    <- S5
...
saveRDS(
  ...
  file = file.path(
    workdir,
    sprintf("GS_synthetic_BGLR_progtest_V2_DD1_rep%03d.rds", rep_id)
  )
)