...
#### Workflow for implementing GS


## =====================================================================
## ARGUMENTS / INTERACTIVE MODE
## =====================================================================
## If you're in R, define rep_id and workdir first; otherwise read commandArgs.
if (exists("rep_id", inherits = FALSE) && exists("workdir", inherits = FALSE)) {
  ...
  if (length(args) < 1) {
    stop("Usage: GS_synthetic_RRBLUPD_realisedbulkprediction_DD1_V2.R <rep_id> [workdir]")
  }
...
model.mse <- function(rep_id) {
  ...
  # Predict EBV using realised bulk genotypes from a synthetic population
  predictEBV_BGLR <- function(pop, tester_pop, bA, bD,
    ...
    for (i in seq_len(n)) {
      ...
      #    using the same mating system as training
  ...
  ## -------------------------------------------------------------------
  ## INITIALISE YEARLY TRAINING STORAGE
  ## -------------------------------------------------------------------
  ...
  for (year in 1:nYears) {
    ...
    if (year == 1) {
      ...
      # These activities are run as if they had been performed during the
      # last few years of burn-in.
      
      # First synthetic varieties created with selected parents from year 17, 18, 19
      # of burn-in. Filling last steps of the pipeline with synthetics derived from
      # these lines.
      ...
      # Use synthetic variety performance data from S4 for training
...
saveRDS(
  ...
  file = file.path(
    workdir,
    sprintf("GS_synthetic_BGLR_realisedbulkprediction_DD1_V2_rep%03d.rds", rep_id)
  )
)