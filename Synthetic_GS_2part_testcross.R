...
if (exists("rep_id", inherits = FALSE) && exists("workdir", inherits = FALSE)) {
  ...
  if (length(args) < 1) {
    stop("Usage: GS_synthetic_Twopart_progenytest_V2_DD1.R <rep_id> [workdir]")
  }
...
model.mse <- function(rep_id) {
  ...
  for (year in 1:nYears) {
    ...
    if (year == 1) {
      ...
      # Use testcross data from S4 (only in first year's cycle) for training
      ...
      # Simulate testcrosses for each training pop ind.
      ...
      # Collect data on testcross performance for added training population
      ...
      # Predict EBV on S2 (only for assessing accuracy)
      ...
      S5twoyearsago <- S5 # No S5lastyear in first cycle
      S5lastyear    <- S5
      ...
      # Use testcross data from S5 last year for training
      S5_training <- replicate(n = 5, expr = { list() }, simplify = FALSE)
...
saveRDS(
  ...
  file = file.path(
    workdir,
    sprintf("GS_synthetic_2part_BGLR_progtest_V2_DD1_rep%03d.rds", rep_id)
  )
)