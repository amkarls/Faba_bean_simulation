...
## Trains prediction models on performance of synthetics derived from each accession
## in training population
## Separate training population per heterotic group
...
model.mse <- function(rep_id) {
  ...
  bulk_codes_from_syn <- function(syn_pop, Nbulk = 20) {
    ...
    A <- 2 * (p_hat - 0.5)
    D <- 2 * p_hat * (1 - p_hat)  # expected het under HWE based on realised p_hat
  ...
  # 2. Calculate genetic distance matrix using Euclidean distance
  ...
  # Synthetic performance as training data (Y0) + realised bulk covariates
  ...
  ## -------------------------------------------------------------------
  ## INITIALISE REALISED-BULK CONTAINERS
  ## -------------------------------------------------------------------
  ...
  # NA-safe cleanup: treat NA effects as 0
  nNA_A <- sum(is.na(bA))
  nNA_D <- sum(is.na(bD))
  if (nNA_A > 0 || nNA_D > 0) {
  ...
  for (year in 1:nYears) {
    ...
    if (year == 1) {
      ...
      # last few years of burn-in
      
      # First synthetic varieties created with selected parents from year 17,18,19
      # of burnin. Filling last steps of pipeline with synthetics derived from
      # these lines
      ...
      # Use synthetic variety performance data from S4 (only in first year's cycle)
      ...
      # Create synthetics from each line, phenotype synthetic bulk and genotype bulk (realised bulk)
      ...
      # Use synthetic variety performance data from S5 two years ago for training
      S5_training <- replicate(n = 5, expr = { list() }, simplify = FALSE)
      for (group in seq_along(S5twoyearsago)) {
        for (line in seq_along(S5twoyearsago[[group]])) {
      ...
      # Realised bulk covariates for this year's training set
      m <- ncol(pullSnpGeno(S5_training[[1]]))
      A_S5_list <- vector("list", length(S5_training))
      D_S5_list <- vector("list", length(S5_training))
...
saveRDS(
  ...
  file = file.path(
    workdir,
    sprintf("GS_synthetic_2part_BGLR_realisedbulkprediction_DD1_V2_rep%03d.rds", rep_id)
  )
)