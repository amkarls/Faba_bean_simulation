...
run_once <- function(run_id){
  with_seed(1000 + run_id, {  # reproducible, different streams per run
    # --- load pkgs & data inside worker ---
    library(AlphaSimR); library(dplyr)
    load("Divpanel_ADG.RData",envir = .GlobalEnv)   # Founder population
    load("SP_ADG.RData",envir = .GlobalEnv)         # Trait architecture
...
for (year in 1:7) {
  ...
  if (year < 7) {
    ...
    for(i in 1:nCrosses){ # Loop over crosses
      F3_i = selectOP(
        ...
        pollenControl = FALSE,
        use = "pheno")
      F3_i = setPheno(F3_i, reps=repF3, varE=varE)###Phenotype headrows in off-season location
  ...
  if (year < 6) {
    ...
    ##Split population by half-sib and collect within-family open-pollinated seeds for each "line" in off-season location
    ...
    for(i in 1:length(motherSval_F4)) {  #Loop over maternal parent
      ...
      F4_split_i = selectOP(
        ...
        pollenControl = FALSE,
        use = "rand") 
      F4_split_i = setPheno(F4_split_i, reps=repF4, varE=varE) ###Corresponds to the phenotype of the F4 in Svalov
  ...
  if (year < 5) {
    ...
    ####Multiply F4 seeds in off-season location to create F5 #### This is not exactly representaative of breeding programme as F3 seeds would be used for the next stage
    ...
    for(i in 1:length(select_F4)){ 
      F5_i = selectOP(
        ...
        pollenControl = FALSE,
        use = "rand")
     F5_i = setPheno(F5_i, reps=repF5, varE=varE) ##########Refers to phenotyping in Svalöv/Sweden of F5s
  ...
  if (year < 4) {
    ...
    ###Multiply F5 in off-season location
    ...
    for(i in 1:length(select_F5)){ 
      F6_i = selectOP(
        ...
        pollenControl = FALSE,
        use = "rand")
      F6_i = setPheno(F6_i, reps=repF6, varE=varE) ############Refers to phenotyping in Svalöv/Sweden at F6 stage
      F6[[i]] = selectInd(F6_i, nInd = 5000, use="rand")
    }
  ...
  if (year < 3) {
    ...
    for(i in 1:length(select_F6)){ 
      F7_i = selectOP(
        ...
        pollenControl = FALSE,
        use = "rand")
      F7_i = setPheno(F7_i, reps=repF7, varE=varE) ############Refers to phenotyping in Svalöv/Sweden at F7 stage
...
for (year in 1:nYears) {
  ...
  ############################
  #create parents##################
  Parents = c(select_F5, F7)### F7 are seeds produced by F6 in off-season location
  ...
  start_pop_DD1 =Parentssel[year==20]
  
}

# Save with run_id suffix
start_pop_DD0 <- Parentssel 
save(start_pop_DD0, file = sprintf("start_pop_ADG_DD0_%d.RData", run_id))
write.csv(output, sprintf("Burninperiod_output_DD0_%d.csv", run_id), row.names = FALSE)
invisible(TRUE)
  })
}

# Launch 10 runs in parallel (files ..._1 to ..._10)
future_map(1:10, run_once, .progress = TRUE, .options = furrr_options(seed = TRUE))
