## =============================================================================
## TRAINING POPULATION DATA
## Based on progeny test followed by synthetic performance
##
## Genomic selection workflow:
## - Split heterotic groups using markers
## - Train prediction models on performance of synthetics derived from each
##   accession in the training population
## - Maintain separate training populations per heterotic group
## - Select parents based on a compound phenotype:
##     focal@pheno[, 1] <- alpha*A + beta*D - gamma*Gbar
##   weighting additive performance, expected dominance, and within-pool
##   heterozygosity
## =============================================================================
...
if (exists("rep_id", inherits = FALSE) && exists("workdir", inherits = FALSE)) {
  ...
  if (length(args) < 1) {
    stop("Usage: GS_synthetic_RRBLUPD_DD1_V2.R <rep_id> [workdir]")
  }
...
model.mse <- function(rep_id) {
  ...
  nself1  = 1
  nself2  = 10
  s2sel   = 200  # number selected from S2 per heterotic group
  ...
  calc_syn_geno <- function(parent_geno, tester_geno) {
    # parent_geno: vector (1 x m)
    # tester_geno: matrix (t x m)
    
    p_parent  <- parent_geno / 2
    p_testers <- colMeans(tester_geno / 2)
    
    # Expected allele frequency in the synthetic
    p_syn <- (p_parent + p_testers) / 2
    
    # Additive code centered at 0
    A <- 2 * (p_syn - 0.5)
    
    # Expected heterozygosity (dominance code)
    D <- 2 * p_syn * (1 - p_syn)
    
    list(A = A, D = D)
  }
  
  # Predict EBV using BGLR additive and dominance marker effects
  predictEBV_BGLR <- function(pop, tester_pop, bA, bD) {
    geno_par  <- pullSnpGeno(pop)
    geno_test <- pullSnpGeno(tester_pop)
    ...
    for (i in seq_len(n)) {
      tmp <- calc_syn_geno(geno_par[i, ], geno_test)
      genoA[i, ] <- tmp$A
      genoD[i, ] <- tmp$D
    }
  ...
  # Genetic relationship / distance matrix
  ...
  # One DH line per tester
  tester_pop <- makeDH(mergePops(tester_list))
  
  
  ## ---------------------------------------------------------------------------
  ## Build initial training population
  ## ---------------------------------------------------------------------------
  
  # Create one inbred line from each accession of start_pop and diversity panel
  ...
  ## ---------------------------------------------------------------------------
  ## Track training and tester populations by year
  ## ---------------------------------------------------------------------------
  ...
  ## ---------------------------------------------------------------------------
  ## Build design matrices across years
  ## ---------------------------------------------------------------------------
  ...
  for (k in seq_along(training_by_year)) {
    ...
    # Genotypes
    geno_par  <- pullSnpGeno(tr_pop)
    geno_test <- pullSnpGeno(tst_pop)
    
    nTrain <- nInd(tr_pop)
    A_blk  <- matrix(NA_real_, nrow = nTrain, ncol = ncol(geno_par))
    D_blk  <- matrix(NA_real_, nrow = nTrain, ncol = ncol(geno_par))
  ...
  # Split population into heterotic groups based on clusters
  parent_groups <- lapply(1:nGroups, function(k) {
    subset(start_pop, start_pop@misc$cluster == k)
  })
  
  # Random selection of individuals used to evaluate selection accuracy
  benchmarking <- lapply(
    parent_groups,
  ...
  for (year in 1:nYears) {
    ...
    if (year == 1) {
      ...
      # These activities are treated as having occurred during the last few
      # years of burn-in, filling the last steps of the pipeline with synthetics
      # derived from year 17, 18, and 19 lines.
      
      # Create SYN0 by crossing all synthetic components
      ...
      # Synthetic performance data from SYN0 used for training GS model
      ...
      # Open pollination of SYN1 to produce SYN2 and phenotype
      ...
      # Select SYN2: random selection because phenotype not yet collected
      ...
      # Split F3 into heterotic groups aligned with parental groups
      ...
      # Collect seeds from selected lines
      ...
      # Self in counter-season location to produce S5
      ...
      ## -----------------------------------------------------------------------
      ## Prepare training data and new parents
      ## -----------------------------------------------------------------------
      
      # Create parents from previous years S3, using tester crosses with
      # synthetics for parental selection and training
      parent_sel = vector("list", length(S3s_sel))
      for (group in seq_along(S3s_sel)) {
        for (line in 1:length(S3s_sel[[group]])) {
          parent_sel[[group]][[line]] = selectInd(
            S3s_sel[[group]][[line]],
            nInd = 1,
            use = "ebv"
          )
      ...
      # Use synthetic variety performance data from S4 (first cycle only)
      ...
      # Simulate open pollination to produce one synthetic line per training pop ind.
      ...
      # Assign phenotypes to mother lines before merging S5 training
      for (group in 1:length(S5_training)) {
        for (individual in 1:length(S5_training[[group]])) {
      ...
      # Collect synthetic performance data for added training population
      ...
      ## -----------------------------------------------------------------------
      ## Rebuild design matrices with matching testers
      ## -----------------------------------------------------------------------
      ...
      ## -----------------------------------------------------------------------
      ## Create new elites / tester lines and benchmark accuracy
      ## -----------------------------------------------------------------------
      ...
      ## -----------------------------------------------------------------------
      ## Create elite lines for synthetics
      ## -----------------------------------------------------------------------
      ...
      # Use synthetic variety performance data from S5 two years ago
      S5_training = replicate(n = 5, expr = { list() }, simplify = FALSE)
      ...
      for (group in 1:length(S5_training)) {
        for (individual in 1:length(S5_training[[group]])) {
          S5twoyearsago[[group]][[individual]]@pheno[, 1] =
            S5_training[[group]]@pheno[individual]
        }
      }
...
saveRDS(
  ...
  file = file.path(
    workdir,
    sprintf("GS_synthetic_BGLR_DD1_V2_rep%03d.rds", rep_id)
  )
)
